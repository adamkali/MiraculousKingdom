...
#[utoipa::path(
    get,
    path = "/api/character/game/{pass}",
    responses((
        status = 200, 
        description = "Found characters for game with pass", 
        body = VecCharDetailedResponse,
    ),
    (
        status = 500, 
...
pub async fn get_characters_by_game(
    Extension(mongo): Extension<Database>,
    Path(pass): Path<String>,
) -> Json<DetailedResponse<Vec<CharacterResponse>>> {
    let mut char_response = DetailedResponse::new(Vec::<CharacterResponse>::new());
    let mut game_response = DetailedResponse::new(Game::new());
    ...
    game_response
        ...
        .run(|a| {
            async {
                a.clone().data.game_chars.iter().for_each(|x| {
                    char_response.data.push(x.as_response());
                });
                a
            }
        })
        .await
        .run(|a| { async { char_response.success = a.clone().success; a } })
        .await;
...
pub mod character_routes {
    ...
    pub use super::get_characters_by_game;
